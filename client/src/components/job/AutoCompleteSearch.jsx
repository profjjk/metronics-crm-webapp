...
const AutoCompleteSearch = ({ customers, setCustomer, setCustomerExists }) => {
  ...
  const onChange = e => {
    const suggestions = customers.map(customer => customer.business_name.toLowerCase());
  ...
  const onClick = e => {
    ...
    setCustomer(customers.filter(customer => customer.business_name.toLowerCase() === e.target.innerText.toLowerCase()));
  }
